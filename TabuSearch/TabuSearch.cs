...
using SLAPScheduling.Tabu_Search;
using System.Data;
...
namespace SLAPScheduling.TabuSearch
{
    public class TabuSearch
    {
        ...
        public List<Location> Implement()
        {
            ...
            this.bestObjectValue = CalculateObjectValue(this.bestSolution);
            ...
            var optimalLocations = GetLocations(this.bestSolution);
        ...
        private Solution InitialSolution()
        {
            ...
            var sortedLocations = this.locationDictionary.OrderBy(x => x.Value.GetDistanceToIOPoint());
        ...
        private IEnumerable<Location> GetLocations(Solution solution)
        {
            ...
            {
                if (this.locationDictionary.TryGetValue(locationIndex, out Location? location))
                {
        ...
        {
            if (currentSolution?.Indices?.Count != this.locationDictionary.Count)
                return 0.0;
        ...
        public (Location Location, ReceiptSubLot SubLot) MapSolutionToSubLot(Solution solution, int index)
        {
            if (solution != null)
            {
                var receiptSubLot = this.receiptSubLots[index];
            ...
            return (new Location(), new ReceiptSubLot());
        }
